...
function _getOrCreateSubFolderByName_(folderNames, parentFolder) {
  let currentFolder;

  if (typeof folderNames == "string") folderNames = [folderNames];
  ...
  for (const folderName of folderNames) {
    ...
    if (folders.hasNext()) {
      ...
      currentFolder = parentFolder.createFolder(folderName);
    }
  }
 ...
 */
function _getSettings_(sheetName = "⚙️Settings", convertKeyToCamelCase = true) {
  const settings = {};
...
function _getFolderIdFromUrl_(url) {
  let regex = /\/folders\/([\w-]+)/;
...
function _generateKeyValuePairFromSheet_(
  sheet,
  startRow = 1,
  keyCol = 1,
  valueCol = 2
  ...
  for (let i = startRow; i < data.length; i++) {
    map[data[i][keyCol - 1]] = data[i][valueCol - 1];
  }

  return map;
}

function _getRowHeights_(range) {
  let sheet = range.getSheet();
  var heights = [];
  for (var i = 1; i <= range.getNumRows(); i++) {
    heights.push(sheet.getRowHeight(range.getRow() + i - 1));
  }
  return heights;
}

function _setRowHeights_(range, heights) {
  let sheet = range.getSheet();
  for (var i = 0; i < heights.length; i++) {
    sheet.setRowHeight(range.getRow() + i, heights[i]);
  }
}

function _showOnlySheet_(sheetToKeep) {
  let sheets = SpreadsheetApp.getActive()
    .getSheets()
    .filter((sheet) => !sheet.isSheetHidden());
  sheetToKeep.showSheet();

  for (let sheet of sheets) {
    if (sheet.getName() !== sheetToKeep.getName()) sheet.hideSheet();
  }
}
...
function _getParsedDataFromSheet_(sheet, filter) {
  let data = _getItemsFromSheet_(sheet)
  ...
  data.forEach((dataR) => {
    ...
    if (dataR.data) {
      try {
        rowData = JSON.parse(dataR.data); // Parse if it contains valid JSON
      } catch (e) {
        console.log(
          `Error parsing projects data in row ${dataR._rowIndex}:`,
          e
        );
      }
    }
    if (!filter || filter(rowData))
      exportData.push({ ...rowData, _rowIndex: dataR._rowIndex }); // Add _rowIndex to the result
  });

  return exportData;
}

function generateKeyUsingUuid() {
  var uuid = Utilities.getUuid();
  var key = Utilities.computeDigest(Utilities.DigestAlgorithm.SHA_256, uuid);
  console.log(Utilities.base64Encode(key));
}
