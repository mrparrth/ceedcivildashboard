import projects from "@/db/JsonData/projects.json";
import payments from "@/db/JsonData/payments.json";
import appData from "@/db/JsonData/appData.json";
import contractMetadata from "@/db/JsonData/contractMetadata.json";
...
export class MockAPI {
  static async executeMockFunction(functionName, { data, token }) {
    ...
    const handlers = {
      ...
      resendInvitationWebsite: () =>
        this.handleMockResendInvitationWebsite(data),
  ...
  static async handleMockCreateUser({
    name,
    email,
    role,
  ...
  static async handleMockUpdateUser(updatedUserInfo) {
    const { id, createdAt, modifiedAt, name, email, role, status } =
      updatedUserInfo;
