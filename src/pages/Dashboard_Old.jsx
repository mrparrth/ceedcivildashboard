...
const Dashboard = () => {
  ...
  return (
    <div className="flex">
      <aside
        id="logo-sidebar"
        className={`fixed top-0 left-0 z-40 w-68 sm:w-65 h-screen transition-transform ${
        ...
        <ul className="flex flex-col items-center sm:items-start">
          {menuItems.map(({ to, icon, label }) => (
            <li key={label} className="w-full p-4">
              <Link
                to={to}
                className="text-white px-0 align-middle flex items-center"
                onClick={toggleSidebar}
              >
      ...
      <div className=" sm:ml-[180px] flex-1 relative">
        ...
        <div className="  rounded-lg dark:border-gray-700 w-full">
          <div className="flex items-center justify-center h-24 mb-4 rounded bg-gray-800 dark:bg-gray-800">
            <h4 className="text-[18px] sm:text-[18px] md:text-[30px] font-semibold text-center text-white flex-grow"></h4>
