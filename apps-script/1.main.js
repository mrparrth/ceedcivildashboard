...
class User {
  ...
  createUser(userInfo) {
    const { name, email, categoryName, userType, merchantName, role, slackEmail, manager } = userInfo
...
class SecureApp extends PublicApp {
  ...
  getSheetData() {
    let { name, role, manager } = this.user;
  ...
  logDifferences(oldObj, newObj, type = 'Project') {
    ...
    Object.keys(newObj).forEach(key => {
      if (['id', 'dateCreated', 'dateModified', 'modifiedBy', 'createdBy'].includes(key)) return; // Skip id field

      const oldValue = oldObj[key];
      const newValue = newObj[key];

      if (Array.isArray(oldValue) && Array.isArray(newValue)) {
        const oldValueStr = JSON.stringify(oldValue.sort((a, b) => {
          if (typeof a === 'object' && typeof b === 'object') {
            return JSON.stringify(a).localeCompare(JSON.stringify(b));
          }
          return String(a).localeCompare(String(b));
        }));
        const newValueStr = JSON.stringify(newValue.sort((a, b) => {
          if (typeof a === 'object' && typeof b === 'object') {
            return JSON.stringify(a).localeCompare(JSON.stringify(b));
          }
          return String(a).localeCompare(String(b));
        ...
        if (oldValueStr !== newValueStr) {
          if (oldValue.length > 0 && newValue.length > 0 && typeof oldValue[0] === 'object' && typeof newValue[0] === 'object') {
            ...
            if (removed.length > 0) {
              changes.push(`Removed: ${removed.map(item => item.name || item.title || item.id || 'item').join(', ')}`);
            }
            if (modified.length > 0) {
              modified.forEach(({ old: oldItem, new: newItem }) => {
                const itemName = oldItem.name || oldItem.title || oldItem.id || 'item';
                const itemChanges = [];
                Object.keys(newItem).forEach(prop => {
                  if (oldItem[prop] !== newItem[prop] && !['id', 'dateCreated', 'dateModified', 'modifiedBy', 'createdBy'].includes(prop)) {
                    itemChanges.push(`${prop}: ${oldItem[prop]} → ${newItem[prop]}`);
                  }
                });
                if (itemChanges.length > 0) {
                  changes.push(`${itemName}: ${itemChanges.join(', ')}`);
                }
              });
            }
            ...
            changeDescriptions.push(`${key}: ${formatArrayForDisplay(oldValue)} → ${formatArrayForDisplay(newValue)}`);
          }
        }
      }
